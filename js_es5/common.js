...
$(function () {
	...
	/*为头部添加meta标签*/
	$.ajax({
		url: "http://192.168.100.250/NavigationMeta/get/" + navId[txtName] + "",
		type: "GET",
	...
	if (navId[txtName] == 1 || navId[txtName] == 4 || navId[txtName] == 11) {
		$.ajax({
			url: "http://192.168.100.250/banner/selectAllByTpye/" + navId[txtName] + "",
			type: "GET",
			success: function success(data) {
...
(function () {
	...
	function jqajax() {
		$.ajax({
			url: "http://192.168.100.250/proCategory/showAll/main", //请求导航头部的产品信息 
			type: "GET",
			success: function success(data) {
		...
		$.ajax({
			url: "http://192.168.100.250/articleCategory/getAll", //请求导航头部的新闻信息 
			type: "GET",
			success: function success(data) {
				data.forEach(function (currentValue, index, arr) {
