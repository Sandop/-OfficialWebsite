...
(function(){
	...
	$.ajax({  
        url: "http://visney.cn:81/product/detail/"+proId+"",  //获取产品详情接口
        type: "GET",  
        success: function (data) { 
        	    ...
        	    PicZoom: function () {
        	        $.fn.PicsZoom = function (o) {
        	            ...
        	            var _this = $(this),
        	                    ...
        	                    liMarginRight = liw - liInW,//计算单个li元素的右边距
        	            ...
        	            if (li > 4) {
        	            	...
        	            	$('#' + nextid).bind("click", function () {
        	            	    if (li >= 4) {
        	            	        ...
        	            	        p.nextmove();
        	            	        setTimeout(function () { btnRight_click_bool = true; $('#' + nextid).bind("click") }, 400);
        	            	    }
        	            	    return false;
        	            	});
        	            } else {
				...
				dataChildren.forEach( function(element, index) {
					prod_link_list_html += `<li class="prod-link-list">
													<a href="prod-list.html?typeId=${currentValue.proCategoryId}&Id=${element.proCategoryId}">
														<span>·</span><span class="prod-link-cont">${element.proCategoryName}</span>
													</a>
												</li>`;
				});
...
(function(){
	var typeId = $.getUrlParam('typeId'),
		Id = $.getUrlParam('Id'); 
	$.ajax({
		url:"http://visney.cn:81/product/getProByTpyeId/"+Id+"/4",
		type:"GET",
		success:function(data){
...
(function(){
	var typeId = $.getUrlParam('typeId'),
		Id = $.getUrlParam('Id'); 
	$.ajax({
		url:"http://visney.cn:81/product/getProRandom/4",
		type:"GET",
		success:function(data){
			data.forEach( function(element, index) {
