...
(function(){
	...
	$.ajax({  
        url: "http://visney.cn:81/proCategory/showAll/list",
        type: "GET",  
        success: function (data) {  
    		//循环遍历家具材质部分内容
...
function getProdAjax(ulBox){
	$(ulBox).delegate("li","click",function(){
		...
		var typeId = $(".furniture-class-kinds-list.active").attr("data-id");  		  //获取选中家具类型data-id
		...
		$.ajax({  
	        url: "http://visney.cn:81/product/selectByCondition/1/"+pageNum+"",
	        method: "POST", 
	        dataType:'json',
	        async:true,
	        data:dataJson, 
...
function getProdPgAjax(nowPage,pageNum){
	...
	$.ajax({  
        url: "http://visney.cn:81/product/selectByCondition/"+nowPage+"/"+pageNum+"",
        method: "POST", 
        dataType:'json',
        async:true,
        data:dataJson, 
